...
void animation();
void back();
int Z = 0, W = 0, V = 0, f=0;                                                                /*________________ All Strucutres ________________*/
 struct flightinfo
{
...
void category(){
    char n;                                // front page of the software
    system("cls");
    printf("\n\t\t\t\t\t\t\t\t\t\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2 MAIN MENU \xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2");
    printf("\n\n\n\t\t\t\t\t\t\t\tPress\n\t\t\t\t\t\t\t\t\t-> 1. Admin\n\t\t\t\t\t\t\t\t\t-> 2. Manager\n\t\t\t\t\t\t\t\t\t-> 3. Passenger\n\t\t\t\t\t\t\t\t\t-> 4. Exit");
    printf("\n\nEnter Choice: ");
    n=getche();
    if (n=='1'){
...
void signinmanager(){
    printf("\n\t \t\t\t\t\t\t~~~~~~~~~~~~~~ SIGN UP OR SIGN IN ~~~~~~~~~~~~~~~\n\n");                   // sign in and sign up option for manager and admin
    printf("\n\t\t\t\t\t\t\t-> 1. Sign In\n\t\t\t\t\t\t\t-> 2. Sign Up\n\n Enter Choice: ");
    int ch;
    ch=getche();
    if (ch=='1'){
...
void registration(){
    printf("\n\t\t\t\t\t\t\t~~~~~~~~~~~~~~ SIGN UP ~~~~~~~~~~~~~~~\n\n");            // registration for new manager and admin
...
void loge(){
    printf("\n\t\t\t\t\t\t\t~~~~~~~~~~~~~~ SIGN IN ~~~~~~~~~~~~~~~\n\n");       // log in option for manager and admin
    int i, found=0,s=0;
...
void savefile(){
    FILE *fp;
    fp= fopen("test.txt", "w");                      // file for saving data. here all the data of the structure will save in test.txt file, and then global variable as test1.txt file.
...
void readfile(){
    FILE *fp;
    fp= fopen("test.txt", "r");
    fread(&access, sizeof(access), 1, fp);     // file for reading data form structure and global variable.
    fclose(fp);
    FILE *fp1;
...
void flightInfo(){
    printf("\n\t\t\t\t\t\t\t~~~~~~~~~~~~~~~~ EDIT FLIGHT INFORMATION ~~~~~~~~~~~~~~~~~");       // flight information update .....
       int i,n;
...
void flightsave(){
     ...
     fwrite(&data, sizeof(data),1,info);       // file used for saving flight information
     fclose(info);
     FILE * info2;
...
void flightread(){
    ...
    fread(&data, sizeof(data), 1, info);   // file used for reading information
    fclose(info);
    FILE * info2;
...
void stuff(){
    printf("\n\t\t\t\t\t\t\t~~~~~~~~~~~~~~~~~~ PAY TO STUFF ~~~~~~~~~~~~~~~~~~~");              // stuff payment functin
    int l,d;
    double sum=0;
...
void stuffsave(){
     ...
     fwrite(&spay, sizeof(spay), 1, stuffpay1);                // file for saving stuff payment record
     fclose(stuffpay1);
     FILE  * stuffpay2;
...
void stuffread(){
     ...
     fread(&spay, sizeof(spay), 1, stuffpay1);              // file for reading stuff payment record
     fclose(stuffpay1);
     FILE  * stuffpay2;
...
void adviewflight(){
        int i,m,no,found=0,found1=0,count=0,count1=0;                                     // funciton for admin to view flight informatin and passenger list
        printf("\n\t\t\t\t\t\t\t~~~~~~~~~~~~ VIEW FLIGHT INFORMATION ~~~~~~~~~~~\n\n");
        printf("\t=====================================================================================================================================");
        printf("\n\tSerial Id\tStatus\t\t\tName\t\tB.Seat\t\tE.Seat\t\tDestination\t\tDate\t\tTime\n");
        printf("\t=====================================================================================================================================\n\n");
        for (i=0; i<Z; i++){         // check if the data is stored...
            found=1;
        ...
        char ch;
        printf("\n\n\nDo You Want To See Passenger List? ( press 'y' For Yes and 'n' For No): ");              // confirm if admin wants to passenger list or not
        ch=getche();
        if (ch=='y'||ch=='Y'){
            ...
            printf("\t\t=======================================================================================================================\n");
            for(m=0; m<V; m++){                 // for viewing all data
                if (no==tckt[m].num){           // for checking respective flight number
                    found1=1;
                    if (tckt[m].type==1){         // to print business class seat
                            printf("\n\t\t%s\t\t\t%d\t\t\t%s\t\t\t%s\t\t\t\tBusiness Class", tckt[m].name1,tckt[m].age,tckt[m].sex,tckt[m].passnum);
                            count++;
                        }
                    else if(tckt[m].type==2){         // to print economy class seat
                            printf("\n\t\t%s\t\t\t%d\t\t\t%s\t\t\t%s\t\t\t\tEconomy Class", tckt[m].name1,tckt[m].age,tckt[m].sex,tckt[m].passnum);
                            count1++;
                        }
                }
            }
            printf("\n\n\n\nBusiness Class Seat Booked : %d", count);            // counter for summing the total booking seats
...
void adviewstuff(){
    ...
    if (n== '1'){
        for (l=0; l<W-1; l++){
            for(j=0; j<W-1-l; j++){
                if (spay[j].sum>spay[j+1].sum){            // bubble sort for ascending order
                    temp=spay[j];
                    spay[j]=spay[j+1];
                    spay[j+1]=temp;
                }
        ...
        for (l=0; l<W-1; l++){
            for(j=0; j<W-1-l; j++){
                if (spay[j].sum<spay[j+1].sum){
                    temp=spay[j];                             // bubbble sort for descending order
                    spay[j]=spay[j+1];
                    spay[j+1]=temp;
                }
            }
...
void adreset(){
    ...
    int i;
    printf("\n\t\t\t\t\t\t\t~~~~~~~~~~~~~~~~~~~~ RESET DATA ~~~~~~~~~~~~~~~~~~~");
    printf("\n\nIf You Reset Your Data You'll No Longer Able to Use Them and Your Program Will Be Closed. Do You Want to Contiue? ('y' for Yes and 'n' for No)\n\nEnter:: ");
    h=getche();
    if (h=='y' || h=='Y'){
        system("cls");
    ...
    if (g== '1'){
        remove("flight1.txt");
        remove("flight2.txt");
        remove("ticket1.txt");                 // to clear all data about flight information and passenger list
        remove("ticket2.txt");
B:      printf("\n\n\n\t\t\t\t\t\t\t   Deleting Data. Please Wait A Few Seconds \n\n\n\n\n\n");
        for(i=0; i<15; i++)
        {
            printf("\r\t\t\t\t\t\t\t\t\t*");Sleep(100); printf("*");Sleep(100); printf("*");Sleep(100); printf("*");Sleep(100); printf("*");Sleep(100); printf("*");Sleep(100); printf("*");Sleep(100); printf("*");Sleep(100);printf("*");Sleep(100); printf("*");Sleep(100); printf("*");Sleep(100); printf("*");Sleep(100);printf("*");Sleep(100); printf("*");Sleep(100); printf("*");Sleep(100);
        }
        system("cls");
        exit(0);
    }
        ...
        remove("stuff1.txt");
        remove("stuff2.txt");          // to clear all data about stuff payment record
        goto B;
    }
    }
    else{
...
void bookticket(){
        ...
        time(&t);
        printf("\n\t\t\t\t\t\t\t~~~~~~~~~~~~ VIEW FLIGHT INFORMATION ~~~~~~~~~~~\n\n");
        printf("\t=====================================================================================================================================");              // for viewing all available flight information
        printf("\n\tSerial Id\tStatus\t\t\tName\t\tB.Seat\t\tE.Seat\t\tDestination\t\tDate\t\tTime\n");
        printf("\t=====================================================================================================================================\n\n");
        for (i=0; i<Z; i++){
            found=1;
        ...
        scanf("%d", & tckt[m].num);
        for(k=0; k<Z; k++){
            if (tckt[m].num==data[k].num){                          // checks if the flight number is available or not
                  found1=1;
                  if (data[k].bseat==0 && data[k].eseat==0){        // checks if the seats are avaibale ro not
                ...
                scanf("%d", & tckt[m].type);
                if (tckt[m].type==1){
                    if(data[k].bseat==0){          // checks if the business class seat is available or not
                        printf("\n\n\t\t>>>>>>>>>> Sorry!!! Business Class Seats Are Not Available <<<<<<<<<<<\n\n");
                        goto A;
                    }
                    else{
                    data[k].bseat--;                // if business class seat is available then decrease one seat
                    }
                }
                else if (tckt[m].type==2){
                    if (data[k].eseat==0){                          // checks if the economy class seat is available or not
                        printf("\n\n\t\t>>>>>>>>>> Economy Class Seat Are Not Available <<<<<<<<<<<\n\n");
                        goto A;
                    }
                    else {
                    data[k].eseat--;                            // if economy class seat is available then decrease one seat
                    }
                }
                printf("\nBooking Date And Time                  : %s", ctime(&t));          // time of booking ticket
...
void ticketsave(){
        ...
        fwrite(&tckt, sizeof(tckt), 1, flight_ticket);                    // file for saving data of the booking ticket
...
void ticketread(){
        ...
        fread(&tckt, sizeof (tckt), 1, flight_ticket);                  // file for reading data of booking ticket
        fclose(flight_ticket);
        FILE * flight_ticket2;
...
void helpcentre(){
    printf("\n\t\t\t\t\t\t\t~~~~~~~~~~~~~~~~~~~~ HELP CENTER ~~~~~~~~~~~~~~~~~~~~~");            // help center for passenger
...
void ticketshow(){
    char tmp_num[10],p;
    printf("\n\t\t\t\t\t\t\t~~~~~~~~~~~~~~~~~~ VIEW BOOKING TICKET ~~~~~~~~~~~~~~~~~~");           // viewieng ticket option for manager to confirm ticket or cancel a ticket
    int m,found=0,b,in;
    printf("\n\nEnter Passenger Passport Number To See The Details: ");
	scanf("%s", tmp_num);
    for(m=0; m<V; m++){
    if (strcmp(tmp_num,tckt[m].passnum)==0){                         // verify the passport number
            found=1;
                ...
                if (strcmp(tmp_num,tckt[m].passnum)==0){
                        for(b=0; b<Z; b++){
                                if (tckt[m].num==data[b].num){
                                        if (tckt[m].type==1){
                                                data[b].bseat++;                      // to increase a business class seat if the ticket is canceled
                                            }
                                        else if(tckt[m].type==2){
                                                data[b].eseat++;                        // to increase an economy class seat if the ticket is canceled
                                            }
                                    }
                            }
                    in=m;
    ...
    flightsave();
    for(m=in;m<V;m++){
    	tckt[m]=tckt[m+1];                     // forwarding one data from the deleting one
	}
	V--;
	ticketsave();
...
void back(){
    char bck;
        printf("\n\nPress 'B' To Go Back: ");
        bck=getche();
        if (bck=='B' || bck=='b'){                                  // a function for going back to previous option
          system("cls");
        }
        else{
            system("cls");
